...
//Переменные
var isUserLoggedIn = document.getElementsByClassName('tab')[0].classList.contains('tab_user');
...
if (isUserLoggedIn) {
    ...
    var trackerNotify = document.getElementsByClassName('navbar_count_tracker_total')[0].innerHTML;
    var dialogsNotify = document.getElementsByClassName('navbar_count_new_messages')[0].innerHTML;
    if (trackerNotify !== '') {
        //Уведомления от трекера
        trackerNotify = '+' + trackerNotify;
    }
    if (isReadOnly === false) {
...
if (window.location.href.indexOf('feed') >= 0) {
    feedActive = 'active';
    postAddButton = '<div class="add_post"><a href="/' + (isReadOnly ? 'sandbox' : 'topic') + '/add"></a></div>';
    ...
    hubsActive = 'active';
} else if (window.location.href.indexOf('companies') >= 0 || window.location.href.indexOf('company') >= 0) {
    compsActive = 'active';
}
...
if (document.getElementsByClassName('similar-posts')[0]) { 
    document.getElementsByClassName('similar-posts__title')[0].classList.add('title');
    document.getElementsByClassName('similar-posts__title')[0].classList.remove('similar-posts__title');
    document.getElementsByClassName('similar-posts')[0].classList.add('block');
    var temp = document.getElementsByClassName('similar-posts')[0].outerHTML;
    document.getElementsByClassName('similar-posts')[0].outerHTML = '';
    document.getElementsByClassName('sidebar_right')[0].innerHTML += temp;
}
